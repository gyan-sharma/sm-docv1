...
SettleMint’s Hasura integration enables a clean separation of concerns in blockchain applications—on-chain for truth and verification, off-chain for user-centric and performance-sensitive features. Developers get instant GraphQL APIs, a relational database, and the flexibility to scale across data sources. This hybrid approach simplifies architecture while maintaining decentralization where it truly matters.

## How to add off-chain database in SettleMint platform
...
![Import](../../../img/blockchain-guide/Hasura_Import.png)

> More info on this and other commands can be found in the
> [Hasura docs](https://hasura.io/docs/latest/api-reference/pgdump/), please
> note to always add the x-auth-token header with an api key for your Hasura
> instance.
