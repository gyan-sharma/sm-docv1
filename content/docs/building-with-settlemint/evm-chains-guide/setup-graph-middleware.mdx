...
Find the chain ID of the network from Blockchain Networks > Selcted Network > Details page, it will be something like **47440**
...
```json
{
  ...
  "datasources": [
    {
      "name": "User",
      "address": "0xd8C9C4Fc1F71869D54fAF8df6b1C5F7421a1e15b",
      "startBlock": 0,
      "module": ["user"]
    }
  ]
...
## 3 Create user.yaml file inside subgraph/datasources/user.yaml 
...
- kind: ethereum/contract
  ...
  mapping:
    ...
    abis:
      - name: User
        file: "{root}/out/User.sol/User.json"
    eventHandlers:
      - event: ProfileCreated(indexed uint256)
        handler: handleProfileCreated
      - event: ProfileUpdated(indexed uint256)
...
## 4. Create user.gql.json, This defines your GraphQL schema (save as subgraph/datasources/user.gql.json):
...
## 5. user.ts (event handlers) Create subgraph/datasources/user.ts:
...
export function handleProfileUpdated(event: ProfileUpdated): void {
  ...
  // Keep existing values until full field update data is included in event
  user.updatedAt = event.block.timestamp
  user.save()
}

export function handleProfileDeleted(event: ProfileDeleted): void {
  const id = event.params.userId.toString()
  let user = UserProfile.load(id)
...
## 6. create subgraph.yaml in the root directory 

```yaml
specVersion: 0.0.5
description: Tracks user profiles (create, update, delete)
repository: https://github.com/your-org/your-repo
schema:
  file: ./generated/user.schema.graphql
  ...
  - kind: ethereum/contract
    ...
    mapping:
      ...
      eventHandlers:
        ...
        - event: ProfileDeleted(indexed uint256)
          handler: handleProfileDeleted
      file: ./subgraph/datasources/user.ts
...
In The Graph’s autogenerated schema, each entity gets two types of queries by default:
	1.	A single-entity query (by ID)
	2.	A multi-entity query (to list/filter all records)

For example, if you have an entity named UserProfile, The Graph automatically generates:
	•	userProfile(id: ID!): Fetch a single UserProfile by its id.
	•	userProfiles(...): Fetch a list of UserProfile entities, with optional filters.

⸻

Why “getProfile” vs. “profiles”?
	•	getProfile(...) likely comes from your contract method (i.e., function getProfile(uint256 userId)) that the subgraph recognized as a read (call) or you explicitly added as a handler in the subgraph.
	•	profiles(...) (or userProfiles) is the autogenerated multi-entity query for your UserProfile entity.

So in your GraphQL explorer, you’ll see both:
	1.	getProfile(userId: ...) – a direct call to the contract function, if you exposed it in your subgraph’s schema or as a call handler.
	2.	profiles(...) (or userProfiles) – the standard “list” query for all UserProfile entities that The Graph generates automatically.

⸻

Do you need both?
	•	If you only want to rely on the indexed data in the subgraph (which is usually the point of The Graph), you can query the subgraph’s standard fields: userProfile(id: ...) and userProfiles(...).
	•	If you still want a direct contract read for “getProfile,” that’s a separate feature. Sometimes people include call handlers for contract read functions, but most subgraphs stick to event-driven data.

In short, “getProfile” is from your contract method, while “profiles” is The Graph’s built-in listing query for your UserProfile entity. They serve similar but not identical purposes:
	•	getProfile returns live data from the contract (if implemented as a call handler).
	•	profiles (or userProfiles) returns data stored/indexed by The Graph from past events.

If you don’t need the direct contract read, you can remove or ignore getProfile. Otherwise, keep both and use whichever suits your data needs.
